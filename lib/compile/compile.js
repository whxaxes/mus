...
class Compiler {
  ...
  processImport(el, scope) {
    const url = computed(el, scope);
    const ast = mus.getAstByUrl(url);
    if (ast.macro) {
      // copy macro to current ast
      const prefix = el.item ? `${el.item}.` : '';
      ast.macro.forEach((macroEl, key) => {
        const macro = this.ast.genMacro();
        macro.set(`${prefix}${key}`, macroEl);
      });
    } else {
...
function include(url, scope) {
  const includeAst = mus.getAstByUrl(url);
  return new Compiler({
    ast: includeAst,
    scope,
  }).compile();
