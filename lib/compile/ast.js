...
class Ast {
  ...
  parse(str) {
    ...
    const chars = str.substring(0, endIndex);
    if (chars.length) {
    ...
    str = this.advance(str, endIndex);

    // get rightEnd index
    endIndex = str.indexOf(rightEnd);
    let expression = str.substring(leftStart.length, endIndex);
      ...
      if (isBlock) {
        if (!isRaw) {
          ...
          if (blockHandle) {
            ...
            element = blockHandle(
              this.genNode(1),
              expression.substring(matches.index + blockName.length).trim(),
              this
            );
          }
          ...
          if (element) {
            ...
            utils.throw(
              `unknown tag '${expression.trim()}'`,
              { _ast: this, _index: this.scanIndex }
            );
          }
        ...
        if (isRaw) {
          ...
          element = processor.variable(this.genNode(3), expression);
        }
      }
    ...
    this.parse(this.advance(str, endIndex));
  }
