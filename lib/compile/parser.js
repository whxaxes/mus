...
exports.parseKeyValue = function(expr, sep, cb) {
  ...
  while (i < fragments.length) {
    ...
    if (frag.expr === sep && sep !== ' ') {
      setValue(str);
      str = '';
    } else if (frag.expr === '=') {
      str = str.trim();
      const lastSepIndex = str.lastIndexOf(sep);
      if (lastSepIndex < 0) {
        key = str;
      } else {
...
exports.parseAttr = function(expr, attrName = 'default') {
  let functionStr = '';
  const result = exports.parseKeyValue(
    expr, ' ',
    (key, value) => (functionStr += `_$s['${key || attrName}'] = ${value};`)
  );
...
exports.parseMacroExpr = function(expr) {
  ...
  if (argsString) {
    ...
    const result = exports.parseKeyValue(
      ...
      (key, value) => {
        if (!key) {
          key = value.replace('_$o.', '');
          value = null;
        }
        setScope += `_$o['${key}']=arguments.length>=${index + 1}?`;
        setScope += `arguments[${index}]:${value || '""'};`;
        index++;
      }
    );
    if (result.complex) {
      throw new Error(`macro arguments error ${argsString}`);
    }
  }
