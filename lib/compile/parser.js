...
exports.parseAttr = function(expr, attrName = 'default') {
  const result = this.parseString(expr, true);
  ...
  while (i < list.length) {
    ...
    if (astNode.expr === '=' && preNode.expr !== '=' && nextNode.expr !== '=') {
      str = str.trim();
...
exports.parseString = function(expr, space) {
  let stringSymbol;
  let i = 0;
  let savor = '';
  let needWith = false;
  ...
  while (i <= expr.length) {
    ...
    } else if (!stringSymbol && (operatorList.indexOf(char) >= 0 || (space && char === ' '))) {
      if (savor) {
  ...
  function formatNewExpr(str) {
    ...
    if (otherRE.test(str)) {
      ...
      // object, pre-append scope object
      newExpr = `_$o.${str}`;
    } else {
      // complex expression, plain object,
      // it's difficult to know which string
      // should be pre-append scope object,
      // so use 'with(){}' should be more easy.
      needWith = true;
    }
    return newExpr;
  }
