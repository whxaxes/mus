...
class Mus {
  ...
  render(url, args) {
    ...
    if (this.async) {
      return new Promise(resolve => {
        compile(this.getAstByUrl(filePath), args, this, resolve);
      });
    } else {
      compile(this.getAstByUrl(filePath), args, this, result => {
        html = result;
      });
  ...
  renderString(html, args) {
    if (this.async) {
      return new Promise(resolve => compile(this.getAst(html), args, this, resolve));
    } else {
      compile(this.getAst(html), args, this, result => {
        html = result;
      });
